...
DATE_MAIL_SUBJECT=`date "+%b %Y" | tr "[:lower:]" "[:upper:]"`
CSV_DIR=/var/log/asterisk/cdr-csv
CSV_FILE=$CSV_DIR/Master.csv

if [ -f $CSV_FILE ]; then
	/root/bin/asterisk_cdr.pl --incoming --outgoing --anonymous --internal > /tmp/asterisk-repport-$DATE.txt
	cat /tmp/asterisk-repport-$DATE.txt | mail -s "Asterisk rapport du mois de $DATE_MAIL_SUBJECT" tuxa@galaxie.eu.org
	rm /tmp/asterisk-repport-$DATE.txt
